...
<html lang="kr">
 ...
 <body>
  <?php
	...
	$sql = "select idx,id,name,title,date from game_developement order by idx desc";
...
<!--lnb end-->
	<div class="content">
		...
		<h4 class="content_title">게임개발자 예비학교</h4>
		</div>
		<table class="board_list">
			...
			?>	
				<tr>
					...
					if($_SESSION['id']==true){	
					?>
					<td class="btitle"><a href="./sh_game_d.php?idx=<?=$row['idx']?>"><?=$row['title']?></td>
					<?php
					}else{
					?>
			...
			if($_SESSION['id']==true){
			?>
				<a href="gd_write.php"><div class="write" align="center">글쓰기</div></a>
			<?php
			}else{
			?>
